...
[<AutoOpen>]
module NugenExtensions =
    type Game with
        member this.GetNugen world = this.GetModelGeneric<Nugen> world
        member this.SetNugen value world = this.SetModelGeneric<Nugen> value world
        member this.Nugen = this.ModelGeneric<Nugen> ()
...
type NugenDispatcher () =
    inherit GameDispatcher<Nugen, NugenMessage, NugenCommand> (Splash)

    // here we define the game's properties and event handling
    override this.Definitions (nugen, _) =
        [Game.DesiredScreen :=
            match nugen with
            ...
            if nugen = Splash then Simulants.Splash.DeselectingEvent => ShowTitle
